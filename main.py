...
def login_tds(token):
    try:
        ...
        if 'success' in r:
            os.system("clear")
            print(f" \033[1;32mLogin success!\n User: \033[1;33m{r['data']['user']}\033[1;32m | Xu hiện tại: \033[1;33m{r['data']['xu']}")
            return 'success'
        else:
            print(f" \033[1;31mToken không hợp lệ, kiểm tra lại!\n")
            return 'error_token'
    except:
        return 'error'
...
if check_log == 'success':
    ...
    while True :
        ...
        max_job = int(input("\033[1;32m Stop delay: "))
        if ss1== 0:
            os.system("clear")
        if choice == 1:
               ...
               for job in list_job['data']:
                    ...
                    if dem_tong == max_job:
                        ...
                        for i in range(delay,-1,-1):
                            print('\033[1;32m Loading : '+str(i)+' sec',end=("\r"))
                            sleep(1)
            if dem_tong == max_job:
                check_out=input(f"\033[1;32mHoàn thành {max_job} nhiệm vụ!\n \033[1;32m Thoát Tool ?(Y/N)")
